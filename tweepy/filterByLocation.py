...
class MyListener(StreamListener):
    ...
    def on_data(self, status):
        note.write(status)           
        tweet=json.loads(status)  #Convert the tweet data to a json object 
...
twitter_stream.filter(locations=[-122.75,36.8,-121.75,37.8],languages=['en'])
